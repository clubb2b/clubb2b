...
import { Users, Shield, Crown, User, History, Search, AlertTriangle } from 'lucide-react';
...
const SecureRoleManagement = () => {
  ...
  const handleRoleChange = async () => {
    ...
    try {
      // First, remove existing role if upgrading/downgrading
      ...
      toast({
        title: "Error",
        description: "Failed to update user role. This action may require additional permissions.",
        variant: "destructive",
      });
    }
  };
    ...
    return (
      <Card>
        <CardContent className="text-center py-8">
          ...
          <p>Loading secure user management...</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const roleStats = {
    ...
    admins: profiles.filter(p => getUserRole(p.id) === 'admin').length,
    vips: profiles.filter(p => getUserRole(p.id) === 'vip').length,
  };
        ...
        <Card>
          <CardContent className="flex items-center p-6">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{roleStats.vips}</p>
            </div>
          </CardContent>
        </Card>
      </div>
                  ...
                  <TableRow key={profile.id}>
                    ...
                    <TableCell>
                      <Dialog open={isDialogOpen && selectedUser?.id === profile.id} onOpenChange={(open) => {
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>Change User Role (Secure)</DialogTitle>
                          </DialogHeader>
                          <div className="space-y-4">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">Current: {currentUserRole.toUpperCase()}</p>
                            </div>
